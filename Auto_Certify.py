...
# add you email and vs code smtp password here, and allow less secure apps in your gmail settings.
# make a password for your vs code smtp, and use that password here.
...
# you can use uncomment this and modify to use a default font
# font = cv2.FONT_HERSHEY_SCRIPT_COMPLEX # Use the script simplex font
# fontscale = 3

# Download the custom font online in this same folder, I used 'GreatVibes-Regular.ttf'
...
for i in range(0, len(nametxt)):
    ...
    print(key)
    if key != 255:
        print(f"Skipped sending the certificate to {nametxt[i]} (key pressed)")
    else:
